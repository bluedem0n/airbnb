...
var mostrarMapa = function (position) {
	...
	autocomplete.addListener('place_changed', function () {
		...
		var contentString = '<img src="{{image}}" width="300px" height="150px">' +
			...
			'<p class="black-text"><strong class="blue-text">New: </strong>{{tittle}}</p></a>' +
			'<div>{{address}}' +
		...
		$.when(
			...
			$.each(lugares[0], function (i, place) {
				largo = lugares[0].length;
				console.log(largo);
				var contentStringResultado = contentString.replace("{{tittle}}", place.title)
					.replace("{{address}}", place.address)
					.replace("{{phone}}", place.contact.phone)
